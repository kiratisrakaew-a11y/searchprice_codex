...
function runMilestone18TpsoRawUpdateContract_() {
  ...
  var apiEndpointOk = getPhase1TpsoApiUrl_() === PHASE1_DEFAULT_TPSO_API_URL || cleanDisplayText_(getPhase1TpsoApiUrl_()).indexOf('/OpenApi/CmiPrice/Month') !== -1;
  var requiredFieldsPresent = findMissingValues_(PHASE1_TPSO_REQUIRED_RESPONSE_FIELDS, Object.keys(apiRows[0])).length === 0;
  ...
  return okResult_({ endpoint: getPhase1TpsoApiUrl_(), raw_sheet: PHASE1_SHEETS.MATERIAL_TPSO, master_clear_forbidden: true });
}
