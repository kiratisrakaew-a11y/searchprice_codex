...
function fetchTpsoMonthlyPrice_(requestBody) {
  var response;
  try {
    var endpoint = getPhase1TpsoApiUrl_();
    ...
    return failResult_(createError_(
      ...
      { endpoint: endpoint, error_message: error.message },
      'critical'
    ));
  }
  ...
  if (statusCode < 200 || statusCode >= 300) {
    return failResult_(createError_(
      ...
      { endpoint: endpoint, status_code: statusCode, response_text: responseText.slice(0, 500) },
      'critical'
    ));
  }
...
function validateTpsoApiRows_(rows) {
  ...
  var requiredFields = PHASE1_TPSO_REQUIRED_RESPONSE_FIELDS;
  var missingByRow = [];
