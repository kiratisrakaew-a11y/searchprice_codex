...
function checkRequiredSheets() {
  ...
  var missingOptionalSheets = PHASE1_OPTIONAL_SHEETS.filter(function(sheetName) {
    return !getSheetByName_(spreadsheet, sheetName);
  });
  ...
  return {
    ...
    optional_sheets: PHASE1_OPTIONAL_SHEETS.slice(),
    missing_sheets: missingSheets,
    missing_optional_sheets: missingOptionalSheets
  };
}
 ...
 * Raw source sheets are existence-checked only and are not auto-created.
 * CHECKLIST_2_SCHEMA is optional because some deploy workbooks omit the documentation sheet.
 */
...
function validatePhase1Sheets() {
  ...
  PHASE1_OPTIONAL_SHEETS.forEach(function(sheetName) {
    var optionalSheet = getSheetByName_(spreadsheet, sheetName);
    if (!optionalSheet) {
      result.warnings.push('Optional sheet missing: ' + sheetName);
      result.sheet_results.push({
        sheet_name: sheetName,
        ok: true,
        optional: true,
        warnings: ['Optional sheet missing']
      });
      return;
    }

    var optionalSheetResult = validateSinglePhase1Sheet_(sheetName, optionalSheet);
    optionalSheetResult.optional = true;
    result.sheet_results.push(optionalSheetResult);
    if (!optionalSheetResult.ok) {
      result.ok = false;
      Array.prototype.push.apply(result.errors, optionalSheetResult.errors.map(function(error) {
        return sheetName + ': ' + error;
      }));
    }
    Array.prototype.push.apply(result.warnings, optionalSheetResult.warnings.map(function(warning) {
      return sheetName + ': ' + warning;
    }));
  });
...
function validateRawSheetHeader_(sheetName, sheet, errors, warnings) {
  ...
  if (sheetName === PHASE1_SHEETS.MATERIAL_TPSO) {
    ...
    var tpsoMissing = findMissingValues_(PHASE1_TPSO_REQUIRED_RESPONSE_FIELDS, tpsoHeader);
    if (tpsoMissing.length) {
      errors.push('TPSO response header row is missing required columns: ' + tpsoMissing.join(', '));
    }
    var tpsoOptionalMissing = findMissingValues_(expected, tpsoHeader);
    if (tpsoOptionalMissing.length) {
      warnings.push('TPSO response header row is missing optional columns: ' + tpsoOptionalMissing.join(', '));
    }
