...
function validateStagingRow_(row) {
  ...
  if (price === null && totalCost === null && materialCost === null && laborCost === null) {
    issues.push(createValidationIssue_('critical', 'price_fields_blank', 'At least one price field must be present'));
  }
  if (price === null && (totalCost !== null || materialCost !== null || laborCost !== null)) {
    issues.push(createValidationIssue_('warning', 'price_blank', 'price is blank; comparison may fall back to available cost fields'));
  }
  if (totalCost === null && (price !== null || materialCost !== null || laborCost !== null)) {
    issues.push(createValidationIssue_('warning', 'total_cost_blank', 'total_cost is blank; comparison may fall back to price or cost fields'));
  }
